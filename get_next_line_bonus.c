...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/24 18:52:17 by akazuki           #+#    #+#             */
/*   Updated: 2023/02/05 16:58:41 by akazuki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	save[OPEN_MAX + 1][BUFFER_SIZE + 1];
	char		*line;
	char		*tmp;
	...
	if (save[fd][0] != '\0')
	{
		tmp = ft_strjoin(line, save[fd]);
		ft_free(&line);
		if (!tmp)
			return (NULL);
		line = tmp;
	}
	return (ft_line_set(save[fd], line, fd));
}
